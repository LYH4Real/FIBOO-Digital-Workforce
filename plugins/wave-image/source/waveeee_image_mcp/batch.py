    ...
    def run(
        ...
        *,
        ...
        with manifest.open("w", encoding="utf-8") as handle:
            with ThreadPoolExecutor(max_workers=concurrency, thread_name_prefix="waveeee") as pool:
        ...
        while True:
            ...
            try:
                response = self.client.generate(
                    ...
                    n=task.get("n", 1),
                    response_format=task.get("response_format", "auto"),
                )
                paths = self.storage.persist(response, safe_filename(task_id))
                return finish({
                    "id": task_id,
                    "success": True,
                    "prompt": prompt,
                    "parameters": {k: task.get(k) for k in ("model", "size", "n", "response_format")},
