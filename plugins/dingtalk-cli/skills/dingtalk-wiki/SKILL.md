...
> FIBOO 分发适配：本 Skill 中的 `dws` 是 `${CODEBUDDY_PLUGIN_ROOT}/bin/dws.exe` 的简称。Windows PowerShell 使用 `& "${CODEBUDDY_PLUGIN_ROOT}/bin/dws.exe"` 后接原参数；优先使用随插件更新的这个版本。账号由每位员工独立登录。
